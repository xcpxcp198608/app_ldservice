...
public class DownloadRemoteApk implements Runnable {

    private static Map<String, RemoteAppInfo> apkInfoMap;
    private static Set<String> downloadingApkSet;

    private static volatile DownloadRemoteApk instance;

    private DownloadRemoteApk(){
        apkInfoMap = new ConcurrentHashMap<>();
        downloadingApkSet = new HashSet<>();
    }

    public static DownloadRemoteApk getInstance(){
        if(instance == null){
            synchronized (DownloadRemoteApk.class){
                if(instance ==null){
                    instance = new DownloadRemoteApk();
                }
            }
        }
        return instance;
    ...
    private void download(final RemoteAppInfo apkInfo) {
        FileUtil.delete(Application.RECOMMENDED_APK_PATH, apkInfo.getName());
                    ...
                    public void onFinished(DownloadInfo downloadInfo) {
                        if(AppUtil.isApkCanInstall(Application.RECOMMENDED_APK_PATH, downloadInfo.getName())){
                            ...
                            String result = execCommand("pm","install","-f",
                                    Application.RECOMMENDED_APK_PATH+"/"+apkInfo.getName());
